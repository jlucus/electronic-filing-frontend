import React, { useContext, useEffect, useState } from 'react';
import Link from 'next/link';
import { useRouter } from 'next/router';
import useSWR from 'swr';
import { ThemeContext } from 'styled-components';
import Moment from 'moment';
import Loader from 'react-loader-spinner';
import 'react-loader-spinner/dist/loader/css/react-spinner-loader.css';
import { HomeLayout } from '../../components/page-layouts';
import {
  Card,
  Col,
  FlexWrapper,
  Row,
  Wrapper,
} from '../../components/layout-containers';
import { Button } from '../../components/buttons';
import { Fieldset } from '../../components/forms';
import { Heading3, PageTitle } from '../../components/typography';
import { DataTable, TaskListStyles } from '../../components/tables';
import { useAuthContext } from '../../components/contexts';
import { endpoints } from '../../constants/endpoints';

const DashboardPage = (props) => {
  const themeContext = useContext(ThemeContext);
  const authData = useAuthContext();
  const router = useRouter();
  const fetcher = (url) => authData.globalFetch(url, 'get');
  const url = endpoints.admin.tasks.getTasks;
  const { data: response, error } = useSWR(url, fetcher);

  if (!response)
    return <Loader color="#078dd3" height={30} type="Bars" width="100%" />;

  if (!(response.success && response.data)) {
    return <h1>Error fetching data</h1>;
  }

  const formattedData = response.data.map((task) => {
    task.created = Moment(task.created).format('MM/DD/YYYY, hh:mm:ss a');
    return task;
  });

  let content = (
    <DataTable
      data={formattedData}
      headers={['Description', 'Created at', 'link']}
      accessors={['task_short', 'created', 'task_link']}
      Styles={TaskListStyles}
    />
  );

  if (formattedData.length === 0) {
    content = (
      <FlexWrapper justifyContent="center" alignItems="center" display="flex">
        <Heading3>You are all caught up!</Heading3>
      </FlexWrapper>
    );
  }

  return (
    <HomeLayout headerMenu={authData.navMenus}>
      <Wrapper
        height="auto"
        className="l-padding-hd l-padding-desktop-td l-padding-desktop-bm"
      >
        <Row>
          <Col span={8}>
            <PageTitle
              title="Admin Dashboard"
              icon={<i className="icon-screen" />}
            />
          </Col>
        </Row>
        <Row>
          <Col span={10}>
            <Card
              bordered
              style={{
                backgroundColor: 'white',
                textAlign: 'left',
                justifyContent: 'start',
                padding: '0.5rem',
              }}
            >
              <div>
                <p>
                  {authData.userProfile && (
                    <>
                      You are logged in as {authData.userProfile.first_name}{' '}
                      {authData.userProfile.last_name} (
                      {authData.userProfile.account_type}
                      ).
                    </>
                  )}
                </p>
              </div>
            </Card>
          </Col>
        </Row>
        <Row margin="2rem 0">
          <Col span={10}>
            <p>
              Go to the <Link href="/admin/admin-testpage">test page</Link>.
            </p>
          </Col>
        </Row>
        <Row margin="2rem 0">
          <Col span={10}>
            <Fieldset padding=".5rem .5rem 0 .5rem">
              <legend>Tasks</legend>
              {content}
            </Fieldset>
          </Col>
        </Row>
      </Wrapper>
    </HomeLayout>
  );
};

export default DashboardPage;
