...
tinycolor.prototype = {
  ...
  getLuminance() {
    ...
    const rgb = this.toRgb();
    let RsRGB; let GsRGB; let BsRGB; let R; let G; let
      B;
    RsRGB = rgb.r / 255;
    GsRGB = rgb.g / 255;
    BsRGB = rgb.b / 255;
  ...
  toHsvString() {
    const hsv = rgbToHsv(this._r, this._g, this._b);
    const h = mathRound(hsv.h * 360); const s = mathRound(hsv.s * 100); const
  ...
  toHslString() {
    const hsl = rgbToHsl(this._r, this._g, this._b);
    const h = mathRound(hsl.h * 360); const s = mathRound(hsl.s * 100); const
...
tinycolor.mostReadable = function (baseColor, colorList, args) {
  ...
  let bestScore = 0;
  let readability;
  let includeFallbackColors; let level; let
    size;
  args = args || {};
